...
export const RaffleModule: React.FC<Props> = ({ activeRaffle, onRaffleRenewed }) => {
  ...
  return (
    <div className="bg-[#FFFDF9] p-8 md:p-10 rounded-[32px] border border-orange-200/80 shadow-md text-center space-y-6 select-none max-w-2xl mx-auto">
      
      {/* Icono Copa */}
      <div className="inline-flex bg-[#FDEFE3] p-5 rounded-full border border-orange-200 shadow-inner">
        <Trophy className="w-12 h-12 text-[#C84B20]" />
      </div>

      {/* Título de la Rifa */}
      <div className="space-y-2">
        <div className="inline-flex items-center gap-2 bg-[#F7F2EB] text-[#8C271E] px-4 py-1.5 rounded-full border border-[#E8DFC8] text-xs font-black uppercase tracking-wider">
          <Calendar className="w-3.5 h-3.5 text-[#C84B20]" />
          <span>{getFormattedDate()}</span>
        </div>

        <h2 className="text-3xl font-black text-[#3D2314] font-brand-display pt-1">
          {activeRaffle ? activeRaffle.nombre : 'Rifa Semanal Activa'}
        </h2>
        <p className="text-xs font-bold text-stone-500 max-w-md mx-auto">
          Al presionar el botón se elige el ganador y automáticamente se inicia el periodo de la nueva semana.
        </p>
      </div>

      {/* Filtro Sucursal */}
      <div className="flex justify-center items-center gap-2 text-sm font-bold text-[#60493A] bg-[#F7F2EB] p-3 rounded-2xl max-w-sm mx-auto border border-[#E8DFC8]">
        <span>Filtrar boletos:</span>
        <select
          value={branchFilter}
          onChange={(e) => setBranchFilter(e.target.value)}
          className="bg-white border border-[#D9CBB5] rounded-xl px-3 py-1 font-bold text-[#3D2314] focus:outline-none cursor-pointer text-sm"
        >
      ...
      <div className="pt-2">
        <button
          ...
          disabled={drawing}
          className="bg-[#3D2314] hover:bg-[#8C271E] text-white font-black text-lg py-4 px-10 rounded-full shadow-lg transform active:scale-98 transition-all flex items-center justify-center gap-3 cursor-pointer disabled:opacity-50 mx-auto"
        >
          <Sparkles className={`w-6 h-6 text-[#F28230] ${drawing ? 'animate-spin' : ''}`} />
      ...
      {showModal && winner && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4 z-50">
          <div className="bg-[#FFFDF9] rounded-[32px] border-2 border-[#C84B20] p-8 max-w-lg w-full text-center space-y-5 shadow-2xl relative animate-in zoom-in-95 duration-200">
            
            <button
              onClick={() => setShowModal(false)}
              className="absolute right-5 top-5 p-2 rounded-full text-stone-400 hover:text-stone-800 hover:bg-stone-100 cursor-pointer"
            >
              <X className="w-6 h-6" />
            </button>

            <PartyPopper className="w-12 h-12 text-[#8C271E] mx-auto" />
            
            <span className="inline-block text-xs font-black text-[#8C271E] uppercase tracking-widest bg-[#FDEFE3] px-4 py-1.5 rounded-full border border-orange-200">
              🎉 Cliente Ganador 🎉
            </span>

            <div className="space-y-1">
              <h3 className="text-3xl font-black text-[#3D2314] font-brand-display">
            ...
            <div className="bg-[#F7F2EB] p-4 rounded-2xl border border-[#E8DFC8]">
              <span className="text-xs text-stone-500 font-bold block mb-1">Ticket Ganador</span>
              <span className="font-mono font-black text-[#C84B20] text-xl bg-white px-4 py-1.5 rounded-xl border border-orange-300 inline-block">
                {winner.ticket}
              </span>
            </div>
            ...
            <div className="flex gap-3 pt-2">
              <button
                ...
                disabled={drawing}
                className="flex-1 bg-[#F7F2EB] hover:bg-[#EFE6D8] font-bold py-3.5 rounded-xl border border-[#D9CBB5] text-[#3D2314] flex items-center justify-center gap-2 cursor-pointer text-sm"
              >
              ...
              <button
                onClick={() => setShowModal(false)}
                className="flex-1 bg-[#3D2314] hover:bg-[#8C271E] text-white font-black py-3.5 rounded-xl flex items-center justify-center gap-2 cursor-pointer text-sm shadow-md"
              >
